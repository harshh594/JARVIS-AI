...
# Arduino Setup
try:
    arduino = serial.Serial(port='COM8', baudrate=9600, timeout=1)  # Change 'COM6' if needed
    time.sleep(2)  # Wait for Arduino to reset
except Exception as e:
    print(f"Error connecting to Arduino: {e}")
    arduino = None  # Arduino not connected
...
def RealtimeSearchEngine(prompt):
    ...
    user_command = prompt.lower()

    # Check for voice control commands
    if "turn on light" in user_command or "light on" in user_command:
        send_to_arduino("turn_on")
        return "Turning on the light."
    elif "turn off light" in user_command or "light off" in user_command:
        send_to_arduino("turn_off")
        return "Turning off the light."

    messages.append({"role": "user", "content": f"{prompt}"})

    # Add Google search results and real-time info to the system chatbot messages
